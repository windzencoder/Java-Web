...
<link href="<%= path %>/resources/css/all.css" rel="stylesheet"
	type="text/css" />
<script src="<%= path %>/resources/js/common/jquery-1.8.0.min.js"></script>
<script src="<%= path %>/resources/js/back/list.js"></script>
</head>
						...
						<tbody>
							...
							<c:forEach items="${messageList}" var="message" varStatus="status">
								...
								<tr <c:if test="${status.index%2 !=0 }">style="background-color:#ECF6EE;"</c:if>>
									...
									<td>
										<a href="#">修改</a>&nbsp;&nbsp;&nbsp; 
										<a href="<%= path %>/DeleteOneServlet.action?id=${message.id}" onclick="if(confirm('确定删除?')==false)return false;">删除</a>
									</td>
								</tr>
